...
После заполнения `.env` можно сразу запускать compose: сервисы сами проверят и применят SQL-миграции при старте.
...
При первом старте `app`, `scheduler` и `worker` автоматически выполняют bootstrap БД. Миграции защищены `pg_advisory_lock`, поэтому параллельный запуск контейнеров безопасен.
...
5. **Обогащение контактов.** Воркер `worker` и оркестратор выбирают компании без контактов. `ContactEnricher` строит список страниц (`/`, `/contact`, `/about` и др.), открывает их через Playwright в headless-режиме, сохраняет текстовый фрагмент главной страницы в `companies.attributes.homepage_excerpt`, извлекает только первый найденный `mailto` email, нормализует его (убирает `mailto:`, угловые скобки, пробелы) и отбрасывает варианты без `@` (телефоны, пустые строки). В БД попадает только валидный адрес.
6. **Генерация писем.** Для каждого email без рассылки оркестратор собирает `CompanyBrief` и `OfferBrief`, затем `EmailGenerator` вызывает OpenAI Responses API. Модель и уровень reasoning берутся из `.env` (`OPENAI_EMAIL_MODEL`, `OPENAI_REASONING_EFFORT`); по умолчанию это `gpt-5-2025-08-07` с `low`. При ошибке выполняются ретраи с задержками, а не отправляется fallback-текст. Ответ парсится по JSON-схеме и возвращает пару `subject`/`body`.
...
7. **Обновление:**
   ```bash
   git pull https://github.com/kodjooo/lead-generation.git
   docker compose up -d --build
   ```
   Новые SQL-миграции будут применены автоматически при старте контейнеров.
8. **Мониторинг:**
   ```bash
...
- `OPENAI_EMAIL_MODEL` — модель OpenAI для Responses API при генерации писем.
- `OPENAI_REASONING_EFFORT` — уровень reasoning (`minimal`, `low`, `medium`, `high`) для Responses API.
- `ENRICH_TIMEOUT_SECONDS`, `ENRICH_MAX_REDIRECTS`, `ENRICH_PROXY_URL` — параметры обхода сайтов при поиске контактов. Используйте прокси, если часть доменов недоступна с сервера; уменьшенный таймаут и лимит редиректов помогают быстрее разбирать backlog. Сейчас enrichment работает через Playwright, поэтому требуется установленный Chromium.
