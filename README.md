...
3. **Запустите сервисы:**
   ...
   docker compose up --build
   ```
7. **Очередь рассылки.** `EmailSender.queue` сохраняет результат генерации в `outreach_messages` со статусом `scheduled`, добавляя случайную задержку 4–8 минут относительно предыдущего письма и гарантируя, что `scheduled_for` попадает в окно 09:10–19:45 по МСК. Email и JSON-запрос LLM кладутся в `metadata`, чтобы можно было восстановить, что именно отправляется.
