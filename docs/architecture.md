...
### Генерация
- `app/modules/generate_email_gpt.py` формирует контекст по компании и предложению, обращается к OpenAI Chat Completions (JSON schema) и делает несколько retry-попыток с прогрессивной паузой между ними (20с, 40с, 60с по умолчанию). Если генерация не удалась, письмо не подставляется из fallback-шаблона, а ошибка поднимается выше.
