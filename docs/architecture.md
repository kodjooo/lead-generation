...
### Отправка
- `app/modules/send_email.py` ведёт очередь писем: `queue` создаёт запись `outreach_messages` со статусом `scheduled`, сохраняет адрес в `metadata.to_email`, вычисляет следующее `scheduled_for` с учётом рандомной задержки 240–480 секунд относительно предыдущего письма (выборка последнего времени идёт с `FOR UPDATE SKIP LOCKED`, чтобы параллельные воркеры не конфликтовали) и нормализует время в окно 09:10–19:45 (Europe/Moscow). В `metadata.llm_request` сохраняется исходный JSON-запрос к LLM.
...
- `app/modules/mx_router.py` выполняет DNS-запрос MX через `dnspython`, хранит результат в in-memory TTL-кэше и классифицирует домены как `RU` / `OTHER` / `UNKNOWN` по набору паттернов и списку форс-доменов.
- Перед доставкой `EmailSender.deliver` проверяет рабочее окно, opt-out и запрашивает `MXRouter`: для `RU` выбирается канал Яндекс, иначе Gmail. Ошибка авторизации Яндекс вызывает фолбэк на Gmail, который фиксируется в метаданных и логах.
...
- `tests/test_mx_router.py` проверяет кэширование MX-записей, форс-домены и обработку ошибок DNS.
- `tests/test_send_email_routing.py` закрывает выбор канала (RU → Яндекс, прочие → Gmail), фолбэк при ошибке авторизации и сохранение `metadata.mx` / `metadata.route`.
...
### Службы Docker
- `app/main.py` запускает оркестратор в режиме `once` или `loop` (CLI аргументы) c отключённым планированием: он выполняет polling → ingest → дедуп → enrichment → постановку писем в очередь (с интервалом 240–480 секунд между письмами) → доставку (только внутри окна 09:10–19:45 МСК).
