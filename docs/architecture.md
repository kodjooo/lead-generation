...
- Параметры обхода (`ENRICH_TIMEOUT_SECONDS`, `ENRICH_MAX_REDIRECTS`, `ENRICH_PROXY_URL`) вынесены в окружение. Это позволяет ускорять деградацию на циклических редиректах и при необходимости ходить к сайтам через HTTP(S) прокси, если домены недоступны напрямую с сервера. `ENRICH_PROXY_URL` может содержать список прокси, разделённый запятыми или переводами строки; `ContactEnricher` сначала пробует direct, затем прокси по хешу URL, а при сбоях временно уводит прокси в cooldown. Сейчас enrichment использует Playwright как единственный механизм загрузки страницы, чтобы сайт рендерился как у обычного пользователя.
