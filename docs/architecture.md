...
- Для `contacts_not_found` добавлен одноразовый backfill-флаг `companies.attributes.contacts_backfill_done_at`. Такие компании могут быть повторно прогнаны после улучшения парсера только один раз; после завершения попытки флаг фиксируется, и дальнейшие циклы их больше не трогают, даже если контакт снова не найден.
