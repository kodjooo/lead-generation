...
### Генерация
- `app/modules/generate_email_gpt.py` формирует контекст по компании и предложению, обращается к OpenAI Responses API (JSON schema через `text.format`) и делает несколько retry-попыток с прогрессивной паузой между ними (20с, 40с, 60с по умолчанию). Модель и reasoning берутся из окружения (`OPENAI_EMAIL_MODEL`, `OPENAI_REASONING_EFFORT`); сейчас дефолт — `gpt-5-2025-08-07` с `low`. Промпт настроен на более короткое и естественное письмо: краткий блок про компанию, без канцелярского тона, с нейтральным CTA без обещаний несуществующих кейсов и без формулировок про “показать примеры”; финал письма предлагает обсудить, уместна ли идея и как подобный сценарий мог бы выглядеть в бизнесе компании, либо какие ещё варианты автоматизации были бы полезны. Если генерация не удалась, письмо не подставляется из fallback-шаблона, а ошибка поднимается выше.
