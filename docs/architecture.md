...
- Перед доставкой `EmailSender.deliver` проверяет рабочее окно, opt-out и запрашивает `MXRouter`: для `RU` выбирается канал Яндекс, иначе Gmail. Ошибка авторизации Яндекс фиксируется как `failed` (без автоматической переотправки через Gmail).
